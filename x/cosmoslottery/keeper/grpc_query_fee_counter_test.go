package keeper_test

import (
	"testing"

	sdk "github.com/cosmos/cosmos-sdk/types"
	"github.com/stretchr/testify/require"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"

	keepertest "github.com/orenshva/CosmosLottery/testutil/keeper"
	"github.com/orenshva/CosmosLottery/testutil/nullify"
	"github.com/orenshva/CosmosLottery/x/cosmoslottery/types"
)

func TestFeeCounterQuery(t *testing.T) {
	keeper, ctx := keepertest.CosmoslotteryKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	item := createTestFeeCounter(keeper, ctx)
	for _, tc := range []struct {
		desc     string
		request  *types.QueryGetFeeCounterRequest
		response *types.QueryGetFeeCounterResponse
		err      error
	}{
		{
			desc:     "First",
			request:  &types.QueryGetFeeCounterRequest{},
			response: &types.QueryGetFeeCounterResponse{FeeCounter: item},
		},
		{
			desc: "InvalidRequest",
			err:  status.Error(codes.InvalidArgument, "invalid request"),
		},
	} {
		t.Run(tc.desc, func(t *testing.T) {
			response, err := keeper.FeeCounter(wctx, tc.request)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
				require.Equal(t,
					nullify.Fill(tc.response),
					nullify.Fill(response),
				)
			}
		})
	}
}
